...
// auto prior for Gaussian statistics
// template<template <typename> class Stats, typename Blocks>
template<typename Stats, typename Blocks>
vector<real_t> autoPrior(
    real_t s2 ,	// desired variance
    real_t p ,	// desired probability of sampling variance below s2
    Emissions<Statistics<Stats, Normal>, Blocks>& y,
    const double noiseStdev	// an estimate of the standard deviation of the noise in the data
) {

	

	y.initForward();
	y.createBlocks( sqrt( 2 * log( ( double )y.blocks().size() ))*noiseStdev );
	SufficientStatistics<Normal> muStats;	// sufficient statistics of observed block means

	while ( y.next() ) {
		for ( size_t dim = 0; dim < y.nrDim(); ++dim ) {
			muStats.addObs( y.suffStat( dim ).sum() / y.blockSize() );	// NOTE sometimes, computations involve dim sum, without increasing the average weight
		}
	}
	size_t N = y.nrBlocks() * y.nrDim();
	double blocksMean = sampleMean( muStats, N );
	double blocksVariance = sampleVariance( muStats, N );
	return NormalInverseGammaAutoPrior( s2, p, blocksMean, blocksVariance );
	
	// TODO we're calculating block mean and block variance here
}
