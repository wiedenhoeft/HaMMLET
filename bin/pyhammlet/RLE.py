...
class RunLengthArray:
	...
	# TODO does this work for multivariate data?
	def compress(self):
		#"""Compress the data as much as possible."""
		
		size = self.ends[-1]
		rle = np.array([(x[0],  sum(1 for _ in x[1])) for x in groupby(self.array)])
		self.ends = self.ends[rle[:,-1].cumsum()-1]
		self.array=rle[:,0]
		del rle
...
def shatter(A, B):
	...
	ends = set()
	assert A.size == B.size, "Cannot shatter RLE of different sizes: %d, %d." % (A.size, B.size)
	ends.update(A.ends)
	ends.update(B.ends)
